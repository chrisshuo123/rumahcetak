<?php
	...
	if(!isset($file)){
		...
		header("refresh:1; url=updateTestimoni.php");
	}
	else{
		...
		if($image_size==FALSE){
			...
			if($kategori == 'pilih'){
				...
				$query = "UPDATE alat SET Judul='$judul',Isi='$isi' WHERE idAlat='$_POST[ID]'";
				echo $query;
			}
			else{
				//Kategori ada yang dirubah
				$query = "UPDATE alat SET Judul='$judul',Isi='$isi', header_alat_idAlat = $kategori WHERE idAlat='$_POST[ID]'";
				echo $query;
			}
			...
			if($kategori == 'pilih'){
				//Kategori tidak ada yang dirubah
				$query = "UPDATE alat SET Judul='$judul',Isi='$isi', image='$image' WHERE idAlat='$_POST[ID]'";
			}
			else{
				//Kategori ada yang dirubah
				$query = "UPDATE alat SET Judul='$judul',Isi='$isi', image='$image', header_alat_idAlat = $kategori WHERE idAlat='$_POST[ID]'";
			}
			//Jika database berhasil terupdate
			if(mysqli_query($con, $query)){
